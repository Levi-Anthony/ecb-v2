// BUILD 5B current-state projection; BUILD 5A historical authority is unchanged.
...
if (
  ...
  new URL(connectionString).port !== "55438"
) {
  throw new Error(
    "BUILD 5B Layer B probes require disposable loopback database",
  );
}
...
async function verifyBoundedExpansion(sql: Sql): Promise<void> {
  ...
  assert(
    counts.transitions === "1" && counts.claims === "3" &&
      counts.links === "1" && counts.referents === "16" &&
      counts.thoughts === "1",
...
try {
  ...
  console.log(JSON.stringify({
    suite: "build-5b-layer-b-standing-history",
    result: "PASS",
    checks: [
