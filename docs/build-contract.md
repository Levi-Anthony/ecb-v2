...
> **NAMING CURRENTNESS — 18 September 2026:** The semantic/compositional successor-orientation function remains without an adopted first-class proper name. ECO-153 qualified **Orienting Composition** only as a semantic proposal/descriptor. ECO-157 recovered the naming provenance and first-class identity requirement. **ECO-158 Shape is now CLOSED PASS** and freezes the naming interface as **first-class proper name + recoverable semantic descriptor**, with procedure/operation, materialization/representation, and machine identifier kept distinct. Hard semantic/structural rejection gates are non-compensatory; first-class identity has its own invocation/memorability/retrieval/durability floor; multiple independent generation lanes must face one common qualification/Overshoot surface; selection is conjunctive rather than score-averaged and preserves `none qualifies`; Principal/human recognition may select only among already-qualified survivors. Historical recovery still governs: **Decoder Ring** was the derivation procedure and **Master Key** the derived room target + helps-rule before later operational blur; the present focal function is the successor synthesis, not one historical key object. **Master Key**, **Principal Governing Orientation / PGO**, generic **Governing Orientation**, and **Orienting Composition** are not canonical first-class proper names. Candidate generation is ready to commission but remains **UNOPENED**. No name adoption, machine identifier, schema/API lexical propagation, runtime mutation, or automatic authority/action implication is authorized. Historical wording below is preserved as lineage rather than silently rewritten.
...
- OB1 is substrate lineage.
- ECB v1 is evidence.
- ECB v2 is greenfield.

Preserve behavior before implementation.

Do not migrate complexity merely because it already exists.

## Layer A — Open Brain evidence substrate

Layer A must remain useful independently of governance.
...
A referent-to-referent relation is expressed as a claim. Do not create a competing relation truth store merely for convenience.

**Evidence Link**  
Stable typed connection between a claim/evaluation and versioned evidence.

**Event**  
Immutable transition/history record.

**Artifact**  
Immutable or versioned representation such as a map, receipt, Master-Key expression, semantic evaluation, envelope, policy, or packet.

Specialized physical structures are promoted only when earned.
...
**Schema promotion**  
Movement from generic representation into dedicated physical structure.
...
Newest does not mean governing.

Before any consequential governance-dependent action is enabled, v2 must implement:
...
Governance state must be reconstructible after restart.
...
Governance retains the conditions that made the envelope legitimate.
