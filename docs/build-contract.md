...
> **NAMING CURRENTNESS — 18 September 2026:** The semantic/compositional successor-orientation function remains without an adopted first-class proper name. ECO-153 qualified **Orienting Composition** only as a semantic proposal/descriptor; ECO-157 recovered naming provenance and the first-class identity requirement; ECO-158 CLOSED PASS froze the naming architecture as **first-class proper name + recoverable semantic descriptor**, with procedure/operation, materialization/representation, and machine identifier kept distinct. **ECO-160 is now commissioned** to execute Generate → Test/Overshoot → Select in a fresh evidence-bearing session. Candidate generation must proceed through independent method lanes before incumbents re-enter, and every candidate faces the same non-compensatory semantic/structural gates, identity floor, retrieval/collision surface, and Overshoot protocol. Selection is conjunctive, preserves `none qualifies`, and permits Principal/human recognition only among already-qualified survivors. Master Key, Principal Governing Orientation / PGO, generic Governing Orientation, and Orienting Composition remain non-canonical and are not generation seeds. ECO-160 is proposal-only: no selected proposal becomes adopted or canonical; no machine identifier, schema/API lexical propagation, runtime mutation, or authority/action implication is authorized. Historical wording below is preserved as lineage rather than silently rewritten.
