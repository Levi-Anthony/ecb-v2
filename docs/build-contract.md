...
> **NAMING / REFERENT CURRENTNESS — 19 September 2026:** ECO-160 remains a valid proposal-only naming return for the successor-orientation referent it tested: **BRIMAR** is analytically qualified there, and **orienting composition** remains its descriptor. Principal review has now exposed a potentially deeper architectural referent, so **adoption is on hold pending ECO-161 Sense**. The provisional integration hypothesis is that URG, PGO/Master-Key orientation, and recursively gated SSMM temporal governance may be irreducible capacities of a more general finite-coherence maintenance mechanism; dynamic pruning, binding, Question Forward, reentry, handoff, Decoder Ring behavior, and FIBERR/Filament continuity may be projections/derived behaviors. None of that is canonical yet. Current Register-B correction governs: do not optimize architecture for smallest/minimum; finite active engagement must not silently reduce dormant semantic possibility. ECO-161 must recover provenance, run ablation/cross-domain tests, and determine whether the earlier naming referent survives unchanged, is a valid projection, or must be reopened upstream. No name adoption, machine identifier, schema/API/tool/runtime rename, historical consolidation, authority implication, action authorization, or runtime mutation follows from this checkpoint.
