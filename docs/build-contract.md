...
> **NAMING CURRENTNESS — 18 September 2026:** The semantic/compositional function referred to historically in this contract as **Master Key** remains under naming rederivation. ECO-153 qualified **Orienting Composition** only as a semantic proposal/descriptor; it did not canonize a first-class proper name. ECO-157 then re-entered Sense because this is a first-class ECOS function requiring a memorable, brand-compatible identity, with SIGMA as the model outcome rather than a mandated acronym form. Historical recovery confirms **Decoder Ring** (derivation procedure) and **Master Key** (derived room target + helps-rule) were originally distinct and later operational usage blurred/expanded that lineage; treat the current focal function as the present successor function, not as one historical key object. **Master Key**, **Principal Governing Orientation / PGO**, generic **Governing Orientation**, and **Orienting Composition** are not canonical first-class proper names. The first-class name remains unresolved pending Shape → Generate → Test/Overshoot → Select and later Principal adoption. Do not infer that the function is one packet, creates authority, equals current binding, or authorizes external effect. No schema/API/runtime lexical propagation is authorized. Historical wording below is preserved as lineage rather than silently rewritten.
