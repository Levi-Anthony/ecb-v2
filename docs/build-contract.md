...
> **NAMING CURRENTNESS — 19 September 2026:** ECO-160 completed Generate → Test/Overshoot → Select under the frozen ECO-158 naming architecture. **BRIMAR** (BRY-mar; proposed expansion **Bounded Relevance Integration for Meaning, Action, and Reentry**) is the selected **proposal** for the first-class proper name; it is **not adopted or canonical**. **Orienting composition** remains the qualified semantic descriptor. Accessible internal exact-collision checks were clean, while the canonical BRAIN semantic-collision check remains **UNKNOWN** because ecb-v2-brain / vezxivrvhakclxuvxzso was not reachable through the available Supabase connection. Principal/human recognition remains open. External unrelated BRIMAR brands/live marks create a public/commercial-use caveat; no trademark/legal clearance is claimed. Master Key, Principal Governing Orientation / PGO, generic Governing Orientation, and Orienting Composition remain non-canonical proper names. Decoder Ring remains the historical derivation procedure. No machine identifier, schema/API/tool/runtime rename, broad lexical replacement, authority implication, action authorization, or runtime mutation follows. The immediate next seam is Principal disposition; adoption, if authorized, must first complete the outstanding canonical semantic-collision check and then use scoped alias/supersession propagation rather than rewriting history.
