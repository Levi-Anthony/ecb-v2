STATUS: SHAPE REQUALIFIED AND CLOSED — 2026-09-05 AMERICA/PHOENIX; MOVE NOT RELEASED
DISPOSITION: DECISION_RECORD
ROLE: Bounded BUILD 5B second examination and Output Contract
AUTHORITY: Explicit human reconciliation/conditional closure authorization; subordinate to hardened governing invariants
...
### Recovered lineage and authority

The original pre-hardening derivation used predecessor
`d2ee85b7fcf38192c94720977e1d51325769ddc0`, tree
`9404280ad44d8d1f7d9532517dbc12dfe974dfba`. Its exact eight-file working state was preserved at
`ea549649ad9993b0f89674227a2846423ce97076`, tree
`889be0d1062a05f16a4d2c06d4953c45f3e1cd32`, on `wip/build-5b-shape-pre-hardening` as
PRE-HARDENING BUILD 5B SHAPE CLOSURE CANDIDATE — NON-AUTHORITATIVE; DO NOT MERGE DIRECTLY.
Its previous closure declarations are historical candidate claims, not authority inherited by copying.

The human RECONCILIATION AUTHORIZATION explicitly permits delta examination, bounded acceptance
correction, and Shape closure if the selected architecture survives. The hardened ancestor is
`0e969b9f72dc198976d7538748a341148edf09b2`, tree
`d736786826dcc45d9cf277d0e5e100c7dcb383e1`. ADR-005 belongs to constitutional hardening;
[ADR-006](../architecture-decisions/006-build-5b-artifact-receipt-boundary.md) now records this local
Artifact/Receipt decision. WT07 keeps its identifier. No implementation or canonical contact occurs
in this reconciliation. Closure is a design disposition, not observed execution PASS.
...
The preserved pre-hardening qualification cited official docs for the mechanics used by the design: [JSON uniqueness testing](https://www.postgresql.org/docs/17/functions-json.html),
...
The pre-hardening record also reports a Supabase changelog review. This delta does not claim a fresh
external-doc or engine verification; installed-version verification and local database probes remain
mandatory at Move entry. No platform mechanism is added by this requalification.
...
2. One local WT07 harness, including the small A1→A2 producer, independent fixed expected mutations,
   same-checker positive/negative controls including P23 with explicit proof-sensitivity evidence,
...
No code, migration, canonical mutation, deployment, or credential change is authorized by this
Shape document. The human separately authorized documentation preservation, hardening, and reconciliation
commits/anchoring. Human Move release must anchor the reviewed Shape tree and define the entry gate.
...
resolved local decisions. ADR-005 promotes only its named narrow syntheses; E07 and E15 are not
wholesale promoted, and this Shape adds no further governing promotion.
...
## Pre-hardening candidate verification record — historical

The following records what the recovered WIP examination checked against d2ee85b, not the
reconciliation diff or a post-hardening engine test. Current delta validation is recorded below.
...
## Constitutional delta requalification — 2026-09-05 America/Phoenix

This is the authorized delta against ADR-005's hardened laws, not a blank-slate redesign. The focal
TR1 episode and Sense corrections survive. K7's representation, checker custody, commit boundary,
constraints, grants, canonical identities, and activation ordering are retained. The result remains
bounded to the service_role producer adversary, with trusted database/DDL/release custody explicit.

| Governing test | Disposition and exact coverage |
|---|---|
| A — No silent promotion | SATISFIED AT DESIGN LEVEL: Q2 scope, Q4 result meanings, Q6 non-propagation, and WT07 P12/P13 keep producer assertions, check results, Claim standing, and governing dimensions separate. PASS cannot designate a current Artifact or accept BUILD 5B. |
| B — Shared seam ≠ shared semantics | SATISFIED AT DESIGN LEVEL: Q1's five native roles share retained payload/identity and integrity, with role-specific obligations. Q6 prohibits open-ended relation assertions or a universal domain-validity kernel. No broader semantics are licensed by the common table. |
| C — Verification modesty | SATISFIED AT DESIGN LEVEL: Q2/Q3/Q5 bind exact input/output/operation, checker identity/version/definition, predeclared obligations, source witness, and component findings. No metaphysical correctness, truth of C, or governance acceptance is claimed. |
| D — Role non-collapse | SATISFIED AT DESIGN LEVEL: the producer asserts success; the checker reads independent persisted source and derives results; technical publication commits them; database/DDL custody is a stated trust assumption; the human alone supplies phase/Move/Metabolize authority. Checker and publisher may share a transaction without becoming the producer or human authority. Q3 privileges enforce the bounded distinctions; no Actor schema is added. |
| E — Proof sensitivity | ACCEPTANCE TIGHTENING REQUIRED, NOW SPECIFIED: P03 detects malformed output and P04/P09 detect bad grounding; they do not directly require preservation=false on otherwise valid output. Added P23 and same-surface control rules. The unchanged positive and known violating output must use the identical installed checker/spec and produce different findings. No runtime result is claimed before execution. |
| F — Adversarial disposition | SATISFIED BY EXPLICIT MATRIX BELOW: all twelve pressures are applicable at bounded strength, each traced to acceptance or a Shape constraint. Broader deferred mechanisms have safe-deferral reasons and explicit expiry routes. |

### Discrepancy D-H1 — acceptance proof-coverage gap

**Classification:** SHAPE_OR_CONTRACT_DEFECT — ACCEPTANCE_SPECIFICATION / PROOF_COVERAGE_SUBTYPE.
This is a documentary omission under the newly governing proof-sensitivity law, not an observed
implementation, tooling, or executable-test failure. The inherited checker already required
preservation comparison; the inherited trace could satisfy its explicit defective-output examples
without demonstrating that component's discriminating power. A constant-true preservation component
would not be exposed by P03's format failure or P04's expected preservation=true alone.

**Correction:** WT07 P23 uses correct A1/source and well-formed A2 with only observed_digest changed to
a valid all-zero digest, producer success true. The same checker must yield preservation=false/FAIL
while the unchanged positive yields preservation=true/PASS, with other components true. The Output
Contract's existing WT07 harness deliverable now explicitly includes this evidence. This selects no
new field, function, trigger, index, privilege, runtime result, canonical fixture, or architecture.

**Disposition:** RESOLVED AT SPECIFICATION LEVEL by the refrozen acceptance obligation. Execution
remains unproven and mandatory at Move's disposable rehearsal. If the actual checker fails this test,
classify that observed failure separately; do not weaken P23. Shape was held open only for this bounded
correction. No material representation/checker/authority/transaction decision was falsified, and no
Sense-level assumption was falsified by this delta examination.

## Standing adversarial-pressure dispositions

The disposition applies to each pressure's reachable behavior in this slice, not every imaginable
mechanism under its name. All twelve are APPLICABLE at that bounded strength. The following obligations
are part of Shape closure and the WT07 acceptance surface; merely naming a pressure does not satisfy it.

| Standing pressure | Disposition | Acceptance obligation / Shape constraint |
|---|---|---|
| duplicate/replay | APPLICABLE | WT07 P05/P06/P18/P20; Q5 exact tuple and immutable derived result. Reused IDs or supplied PASS reject; a new observation recomputes under a new attempt. |
| concurrency | APPLICABLE | P10/P14/P16/P17; Q5 no mutable head, previously committed marker, one terminal receipt per attempt. Assert winner-commit and winner-rollback branches separately with no orphan Referent. |
| stale state/basis | APPLICABLE | P04/P09/P19/P21; Q2 historical Event/Link grounding and Q5 checker binding. Wrong historical facts fail; unsupported checking basis is incomplete. Old input is not stale merely by age. |
| partial failure/rollback | APPLICABLE | P07/P11/P14/P15/P17; Q5 distinct schema/fixture/receipt atomic boundaries and durable attempt. No rolled-back output/result can masquerade as committed success. |
| restart/reconstruction | APPLICABLE | Required fresh-connection reconstruction plus P02/P08/P15/P18. Recover exact bytes/spec/witness and distinguish missing attempt, nonterminal attempt, and terminal observations. No newest-row shortcut. |
| wrong identity/role | APPLICABLE | P06/P08/P12/P20/P22; Q1 native participation, Q3 producer/checker/custody/human distinctions, Q6 no identity/semantic promotion. Wrong available output participant fails; unavailable native target is incomplete. |
| wrong version | APPLICABLE | P05/P06/P07/P19/P20; exact immutable IDs/digests, format/version literals, retained checker definition and installed binding. No output replacement or checker substitution under an old result. |
| basis drift/revocation | APPLICABLE | P04/P09/P19/P21; changes to historical source facts or checking definition are observed mismatch/unavailability, not automatic upstream mutation. Q2 excludes present Thought/Claim state from replacing historical basis. General authority revocation is not activated; see L2 below. |
| checker bypass/false PASS | APPLICABLE | P03/P04/P05/P09/P13/P14/P19/P20/P23; Q3/Q5 prohibit caller result custody and enforce actual computation. Positive/negative controls distinguish format, grounding, and preservation sensitivity. |
| retry/idempotency | APPLICABLE | P06/P15/P17/P18 and frozen stage recovery; unique terminal observation, exact-ID read after uncertain commit, no overwriting or duplicate stage insertion. No universal exactly-once delivery claim. |
| time/order ambiguity | APPLICABLE | P14/P15/P16; Q4 top-level transaction distinction, Q5 commit confirmation, and scoped snapshot observations. recorded_at is not commit order/currentness; XID is not global order or identity. |
| unauthorized mutation | APPLICABLE | P01/P05/P07/P12/P20/P22; Q5 explicit column/role grants, trigger-only derivation, immutable protections, no existing grant changes or Claim updates. Owner/DDL adversary is an explicit unproved limit, not a claimed passing defense. |

### Scoped deferral leases — no implicit implementation expansion

These defer broader mechanisms, not the applicable bounded obligations above. Each has a safe-deferral
reason and an expiry condition with a named route. The Build Contract permits deferral only on those
terms; expiry requires renewed disposition before the affected decision is relied on.

| Lease / sub-scope | Disposition | Why deferral is currently safe for this contract | Expiry condition and route |
|---|---|---|---|
| L1 — Cross-cluster recovery and global transaction ordering | DEFERRED | WT07 requires same-installed-database fresh-context recovery; no import/restore, cross-cluster XID continuity, or disaster-recovery guarantee is claimed. Current local restart/rollback obligations remain applicable. | Before any logical import, cluster replacement, or recovery plan relies on copied created_xid or old checker bindings: reopen Q4/Q5 through BUILD 5B Shape requalification. No transparent reuse of those execution assumptions. |
| L2 — Governing basis revocation and automatic standing propagation | DEFERRED | This receipt describes historical Event/Link correspondence and checking capability; it neither grants warrant nor depends on governing currentness. It cannot authorize action or mutate Claim standing. | First proposed use of receipts to authorize action, govern currentness, or propagate qualification: route to the authorized governance Build and AP-01/AP-04, plus AP-07 if a new relation is required, before that use. |
| L3 — External custody attestation and adversarial owner/DDL protection | DEFERRED | Trusted database/DDL/release custody is declared in the operation spec and proof limits. The current adversary is service_role; no self-hosted receipt claims to defeat an owner rewriting its evidence. | Evidence invalidating that trust assumption, or a request to use a receipt across an untrusted custody boundary: suspend that reliance and reopen Q3/authority boundary for human disposition before installation/use. |
| L4 — General distributed retry orchestration / eventual completion | DEFERRED | The local attempt/receipt boundary already preserves uncertain outcomes and duplicate rejection; it promises neither automatic resumption nor eventual completion. No queue, worker, or multi-service delivery contract exists. | First requirement for an autonomous retry worker, distributed delivery, or a liveness guarantee: reopen Q5 and bind that contract before activating it. |

No new aperture, Artifact DAG, Actor ontology, bitemporal model, universal current-state projection,
universal relation/transition kernel, disaster-recovery architecture, or universal idempotency mechanism
is selected here. The existing local UTF-8 SHA-256 choice stays local to K7; ADR-005 does not endorse
an algorithm. Shared Artifact storage does not become a domain-validity register.

## Requalified closure disposition

The human's reconciliation authorization permits bounded acceptance tightening followed by closure.
Documentary checks passed. Shape is closed on the hardened ancestor with corrected WT07
and the existing Output Contract plus its explicit sensitivity evidence requirement. BUILD 5A stays
closed; Sense is not reopened; BUILD 6 and Move remain unopened. No runtime or canonical mutation is
performed by this reconciliation, and its design findings are not executable PASS evidence.

Current delta verification:

- The five constitutional-hardening surfaces remain byte-identical to the hardened ancestor, except
  the ADR index receives the separately authorized ADR-006 row. Historical evidence standing remains
  unchanged; no local Shape text overrides those governing laws.
- K7 Q1–Q6 and the enforcement mapping remain byte-identical to the WIP. The five Output Contract
  deliverables are retained; the existing harness obligation adds P23 sensitivity evidence only.
- WT07's original P01–P22 and nine reserved identities/literal payloads remain unchanged. P23 is the
  sole new challenge; the disposable probe adds no canonical fixture or runtime implementation.
- Independent document parsing confirms that the P23 digest is well-formed and differs from the
  constant witness while all other representation facts remain unchanged. This is not execution of
  the future SQL checker. Format and grounding failure cannot substitute for preservation=false.
- The twelve exact pressure names each have APPLICABLE disposition and a mapped obligation; each
  broader DEFERRED lease includes a safety reason and explicit expiry route. None silently licenses
  a universal mechanism.
- Local document links, challenge references, frozen historical acceptance prefix, protected-file
  identity, and whitespace were checked. No historical harness/receipt, SQL, runtime, server, MCP,
  deployment, or database state was changed or executed.

**Result:** bounded acceptance tightening, D-H1 resolved at specification level; selected architecture
and material Output Contract scope unchanged; Sense not reopened. SHAPE=CLOSED_ON_HARDENED_LINEAGE;
MOVE=UNRELEASED. The reconciliation commit/diff is the reviewable Shape closure; human Move release
remains a separate subsequent action.
