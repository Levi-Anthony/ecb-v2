...
- No dedicated Kernel Packet physicalization has been earned. AP-05 remains open and must be tested against concrete integrity, lifecycle, query, transaction, reconstruction, and bypass liabilities.
- A successful specimen must be stronger than serialization/file transfer: it requires a useful positive meaning-sensitive conclusion and a discriminating semantic negative control through the same bounded checking surface.
...
Active Shape-facing questions include:

- exact destination question / first specimen that demonstrates propagation beyond BUILD 9;
- minimum carried-versus-referenced kernel and dependency closure;
- minimum destination-retained content and source-loss behavior;
- independent source / projection / version / focal identity binding;
- meaning-sensitive comparator/checker basis and discriminating controls;
- AP-05: smallest enforcement on existing Artifact machinery versus any earned dedicated representation;
- durable partial-write / rollback / interrupted-check semantics;
- replay, duplicate, uncertain-retry, and changed-basis behavior;
- destination-local applicability/current reliance observations and HOLD behavior;
- bounded predecessor/order facts without global time semantics;
- cold-entry locator/context contract and discoverability requirements;
- exact trigger status of inherited BUILD 5B L1–L4 leases;
...
- a dedicated Kernel Packet primitive/table;
- canonical database/runtime installation;
- real Master-Key designation or real action authority/effect;
- automatic downstream invalidation/requalification;
- global clocks or generalized temporal coordination;
- actor merge/adjudication or universal semantic consensus;
- governed self-amendment;
- event bus/message broker/distributed-systems ontology;
- production deployment or post-BUILD-10 programs.
